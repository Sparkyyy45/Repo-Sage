...
export function RateLimitBanner({ info }: { info: RateLimitInfo }) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm">
      ...
      <div>
        <p className="font-medium text-amber-800">GitHub API rate limit nearly reached</p>
        <p className="mt-0.5 text-amber-700/80 text-xs">
          {info.remaining} of {info.total} requests remaining. Resets in {formatResetTime(info.reset)}.
